...
module Spider::Messenger
      ...
      module Mobyt
            ...
            def self.parametri(username,password,to,from,testo,operation="TEXT",udh="")
                  #testo_codificato = self.to_gsm0338(testo)
                  testo_codificato = Iconv.iconv('ASCII//TRANSLIT', 'UTF-8', testo)
            ...
            def self.trasforma_accenti(stringa)
                  accents = { 
                    ['á','à'] => "a'",
                    ['À']     => "A'",
                    ['é','è'] => "e'",
            ...
            def self.to_gsm0338(text)
                  latin1_to_gsm_map = Hash.new
                  GSM0338_MAP.each_slice(2) { |gsm_symbol, latin1_symbol| latin1_to_gsm_map[latin1_symbol] = gsm_symbol }
                  val = Iconv.iconv('ISO-8859-1', 'UTF-8', text).first.unpack('C*')
                  val.collect { |utf8_symbol| latin1_to_gsm_map[utf8_symbol] }.flatten.pack('c*')
            end

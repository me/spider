...
function setValue(target, field, value) {
	if ( target.nodeType ) {
	    if (field == 'val' && $(target).is('input[type=checkbox]')){
	        if (value) $(target).attr('checked', true);
...
function getMapping(ev, changed, newvalue, map, source, target) {
	...
	var val = null;
	if (isSetData) val = newvalue;
	else if (target && $(target).is('input[type=checkbox]')){
...
$.extend($.fn, {
	link: function(target, mapping) {
		...
		for (var n in target){
		    if (!$.isFunction(target[n])){
                handlerRev({type: 'changeData'}, n, target[n]);
		    }
		}
		return self;
	},

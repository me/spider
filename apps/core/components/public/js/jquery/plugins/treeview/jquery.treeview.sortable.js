...
;(function($) {
	...
	$.widget("ui.sortableTree", $.extend($.ui.mouse, {
		...
		_intersectsWithEdge: function(item) {	
			...
			if(!this._intersectsWith(item.item.parents(".ui-sortableTree").data("sortableTree").containerCache)) return false;
				...
				if(this._intersectsWith(this.containers[i].containerCache)) {
					if(!this.containers[i].containerCache.over) {
						...
						if(this.currentContainer != this.containers[i]) {
							...
							itemWithLeastDistance ? this._rearrange(e, itemWithLeastDistance) : this._rearrange(e, null, this.containers[i].element);
							this._trigger("change", e, this._uiHash(this)); //Call plugins and callbacks
							this.containers[i]._trigger("change", e, this); //Call plugins and callbacks
							this.currentContainer = this.containers[i];

						}
						
						this.containers[i]._trigger("over", e, this);
						this.containers[i].containerCache.over = 1;
					}
				} else {
					if(this.containers[i].containerCache.over) {
						this.containers[i]._trigger("out", e, this);
		...
		_mouseStop: function(e) {
			...
			this._trigger("stop", e); //Call plugins and trigger callbacks
			...
			if( (this.position.dom != this.currentItem.prev()[0]) ||
			    (this.position.parent != this.currentItem.parent()[0])) this._trigger("update", e, this._uiHash(this)); //Trigger update callback if the DOM position has changed
			if(!this.element[0].contains(this.currentItem[0])) { //Node was moved out of the current element
				...
				for (var i = this.containers.length - 1; i >= 0; i--){
					if(this.containers[i].element[0].contains(this.currentItem[0])) {
						this.containers[i]._trigger("update", e, this);
						this.containers[i]._trigger("receive", e, this);
					}
				};
			};
			...
			for (var i = this.containers.length - 1; i >= 0; i--){
				this.containers[i]._trigger("deactivate", e, this);
				if(this.containers[i].containerCache.over) {
					this.containers[i]._trigger("out", e, this);
					this.containers[i].containerCache.over = 0;
				}
		...
		_mouseDrag: function(e) {
			...
			if(intersectsWithDroppable) {
				...
				for (var i = this.items.length - 1; i >= 0; i--) {
					...
					this._rearrange(e, this.items[i]);
					this._trigger("change", e); //Call plugins and callbacks
					break;
				}
			}
			...
			this._trigger("sort", e); //Call plugins and callbacks
		...
		_uiHash: function(inst) {
    		...
    		return {
				...
				absolutePosition: self.position.absolute,
				instance: this,
